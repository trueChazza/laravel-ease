...
var require_cjs = __commonJS((exports2) => {
  ...
  var normalizePath = function(filename) {
    return filename.split(path4.win32.sep).join(path4.posix.sep);
  };
  function getMatcherString(id, resolutionBase) {
    if (resolutionBase === false || path4.isAbsolute(id) || id.startsWith("*")) {
      return id;
    }
    const basePath = normalizePath(path4.resolve(resolutionBase || "")).replace(/[-^$*+?.()|[\]{}]/g, "\\$&");
...
var import_pluginutils = __toModule(require_cjs());
...
function createDescriptor(filename, source, root, isProduction) {
  const {descriptor, errors} = import_compiler_sfc.parse(source, {
    filename,
    sourceMap: true
  });
  const normalizedPath = import_slash.default(import_path.default.normalize(import_path.default.relative(root, filename)));
  descriptor.id = import_hash_sum.default(normalizedPath + (isProduction ? source : ""));
...
function compile(code, descriptor, options, pluginContext, ssr) {
  ...
  const result = import_compiler_sfc2.compileTemplate({
...
function resolveTemplateCompilerOptions(descriptor, options, ssr) {
  ...
  if (options.devServer) {
    if (filename.startsWith(options.root)) {
      assetUrlOptions = {
        base: options.devServer.config.base + import_slash2.default(import_path2.default.relative(options.root, import_path2.default.dirname(filename)))
      };
    }
  } else {
    assetUrlOptions = {
...
function resolveScript(descriptor, options, ssr) {
  ...
  let resolved = null;
  resolved = import_compiler_sfc3.compileScript(descriptor, {
    ...options.script,
    id: descriptor.id,
    isProd: options.isProduction,
...
var debug = import_debug.default("vite:hmr");
...
var exportDefaultClassRE = /export\s+default\s+class\s+([\w$]+)/;
...
const _sfc_main = ${classMatch[1]}`;
        ...
        scriptCode = import_compiler_sfc4.rewriteDefault(script.content, `_sfc_main`);
      }
      map = script.map;
      if (script.lang === "ts") {
      ...
      const langFallback = script.src && import_path3.default.extname(src).slice(1) || "js";
...
async function transformStyle(code, descriptor, index, options, pluginContext) {
  ...
  const result = await import_compiler_sfc5.compileStyleAsync({
    ...options.style,
...
function vuePlugin(rawOptions = {}) {
  ...
  const filter = import_pluginutils.createFilter(rawOptions.include || /\.vue$/, rawOptions.exclude);
  return {
    name: "vite:vue",
    handleHotUpdate(ctx) {
      if (!filter(ctx.file)) {
